...
### Bước 5: Cài Đặt GWS CLI (Công Cụ Lõi Của Windows)
1. Hãy mở thư mục tài liệu khóa học hoặc link Thư viện của Giảng viên cấp, tìm và tải xuống file công cụ có tên là `gws.exe` (Dành cho Windows).
2. Khi đã tải về máy, **Cách Dễ Nhất** cho người chưa từng học IT là: Bạn hãy Copy (Sao chép) cái file `gws.exe` đó.
3. Dán (Paste) vứt thẳng nó nằm cạnh chung thư mục chứa file code `gdrive_workflow.py` này.
4. *(Tùy chọn cho người biết dùng)*: Nếu bạn muốn gọi `gws` ở mọi thư mục trên máy, hãy đưa đường dẫn của file `gws.exe` vào System Environment Variables (biến `PATH`) của Windows.
